...
const handelsearch = e => {
  ...
  return (
    <article className='fhather'>
        ...
        <section className='Tem_Image'>
            <img src={`https://openweathermap.org/img/wn/${app.weather[0].icon}@4x.png`} alt="" />
            
   <h2 className='Name'><span>City: </span>{app?.name}, {app?.sys.country}</h2>
        </section>
   ...
   <div>
        <section className='info_ini'>
            ...
            {   
                change
                ?<h2 className='Number'>{temperature?.celsius}°C</h2>
                :<h2 className='Number'>{temperature?.farenheith}°F</h2>
            }
             <button id='temp' onClick={handelChangeTemperature}>Change to <span className='G'>{change ? '°F' : '°C'}</span> </button>
            </nav>
